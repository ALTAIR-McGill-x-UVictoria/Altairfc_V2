...
beacon's default. read_current_a() is exposed for telemetry/logging only.

Open-loop only: no current-hold PI loop, since the beacon's job is visual
strobing, not calibrated optical output -- the brightness setpoint is a fixed
DAC code (config/settings.toml's [tasks.lighting].beacon_dac_code), not a
current target. Must share the same LedBoard as SphereLedSource, which is the
single writer to this MCP4728 -- never open a second MCP4728Driver against
this chip. The sphere and beacon can be energized simultaneously (no
electrical constraint); keeping the beacon off during actual sphere
calibration exposures is a scheduling concern handled by
...
Usage:
    ...
    beacon.set_brightness(2800)   # clamped to BEACON_MAX_SAFE_CODE regardless
...
# the sphere source. The new LED driver hardware needs at least 2100 to
# produce any output at all (see BEACON_MIN_CODE) and no separate measured/
# rated maximum exists for this LED yet, so this equals MAX_CODE (the DAC's
# own 12-bit limit) -- replace with a real measured ceiling once one exists.
BEACON_MAX_SAFE_CODE = MAX_CODE

# Minimum DAC code needed for the new LED driver hardware to produce any
# output at all -- a beacon_dac_code below this in config/settings.toml would
# leave the beacon effectively (or fully) dark despite the relay being
# energized. set_brightness() does not clamp up to this floor (a caller may
# legitimately want the beacon off-but-armed at code 0 between flashes in
# some future use), but logs a warning if asked for a nonzero code under it.
BEACON_MIN_CODE = 2100
...
class BeaconChannel:
    ...
    def set_brightness(self, code: int) -> None:
        ...
        """
        clamped = max(0, min(BEACON_MAX_SAFE_CODE, int(code)))
        if 0 < clamped < BEACON_MIN_CODE:
            logger.warning(
                "BeaconChannel: brightness code %d is below BEACON_MIN_CODE (%d) — "
                "the LED driver may produce little or no light at this code",
                clamped, BEACON_MIN_CODE,
            )
        self._target_brightness = clamped
        if self.lit:
